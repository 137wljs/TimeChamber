...
class MA_Ant_Bug_Battle(VA_VecTask):
    ...
    def __init__(self, cfg, sim_device, rl_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.move_scale = 0. # 位移奖励,在课程二：向对方位移时改为0,课程一：向圆心位移时为10
...
def compute_agent_reward(
    ...
    ant_rush_to_op_reward = torch.maximum(ant_rush_to_op, torch.zeros_like(ant_rush_to_op)) * 10. # 目前位移奖励系数是10,在课程一：向圆心位移时改为0
    ...
    bug_rush_to_op_reward = torch.maximum(bug_rush_to_op, torch.zeros_like(bug_rush_to_op)) * 10. # 目前位移奖励系数是10，在课程一：向圆心位移时改为0
