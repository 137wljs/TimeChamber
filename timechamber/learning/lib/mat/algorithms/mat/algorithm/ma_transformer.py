...
from timechamber.learning.lib.mat.algorithms.utils.util import check, init
from timechamber.learning.lib.mat.algorithms.utils.transformer_act import discrete_autoregreesive_act
from timechamber.learning.lib.mat.algorithms.utils.transformer_act import discrete_parallel_act
from timechamber.learning.lib.mat.algorithms.utils.transformer_act import continuous_autoregreesive_act
from timechamber.learning.lib.mat.algorithms.utils.transformer_act import continuous_parallel_act

from timechamber.learning.lib.core.running_mean_std import RunningMeanStd, RunningMeanStdObs
...
class Encoder(nn.Module):
    ...
    def forward(self, state, obs):
        ...
        v_loc = v_loc.reshape(-1, 1) # reshape成原ant_bug框架需要的形状
...
class MultiAgentTransformer(nn.Module):
    ...
    def __init__(self, state_dim, obs_dim, action_dim, n_agent,
                 n_block, n_embd, n_head, input_shape, encode_state=False, device=torch.device("cpu"),
                 action_type='Discrete', dec_actor=False, share_actor=False, normalize_value=True, normalize_input=True):
        ...
        self.value_size = 1
        # 原mlp用来归一化的，具体可能还要修改一下
        # mat可能也归一化了但是不在这个地方，具体细节还得改一改
        ...
        if self.normalize_input:
            if isinstance(input_shape, dict):
                ...
                self.running_mean_std = RunningMeanStdObs(input_shape[0])
            else:
                self.running_mean_std = RunningMeanStd(input_shape[0])
    ...
    def forward(self, state, obs, action, available_actions=None): # 训练 obs: (n_rollout_thread * episode_length // num_mini_batch, n_agent, obs_dim) n个环境收集m条数据，分成mini_batch组，每组n*m // mini_batch条数据用来更新网络
        # state: (batch, n_agent, state_dim)
        # obs: (batch, n_agent, obs_dim)
        ...
        if self.action_type == 'Discrete':
            ...
            action_log, entropy, mus, sigmas = continuous_parallel_act(self.decoder, obs_rep, obs, action, batch_size,
                                                          self.n_agent, self.action_dim, self.tpdv)

        # return action_log, v_loc, entropy
        neglogpacs = -action_log.sum(-1).reshape(-1)
        
        # 返回值变成dict类型适应原ant_bug框架,原mat值都在cpu上因为环境要求，使用.to(**self.tpdv)转到gpu上
        result = {
            'prev_neglogp': neglogpacs.to(**self.tpdv),
            'values': v_loc.to(**self.tpdv), 
            'entropy': entropy.to(**self.tpdv),
            'mus' : mus.to(**self.tpdv),
            'sigmas' : sigmas.to(**self.tpdv)
        }

        return result

    def get_actions(self, state, obs, available_actions=None, deterministic=False): # 采样，obs: (n_rollout_threads, n_agent, obs_dim)
        # state unused
        ...
        if self.action_type == "Discrete":
            ...
            output_action, output_action_log, mus, sigmas = continuous_autoregreesive_act(self.decoder, obs_rep, obs, batch_size,
                                                                             self.n_agent, self.action_dim, self.tpdv,
                                                                             deterministic)
        
        # return output_action, output_action_log, v_loc
        neglogpacs = -output_action_log.sum(-1).reshape(-1)
        # 返回值变成dict类型适应原ant_bug框架,原mat值都在cpu上因为环境要求，使用.to(**self.tpdv)转到gpu上
        
        result = {
            'neglogpacs': neglogpacs.to(**self.tpdv),
            'values': self.denorm_value(v_loc).to(**self.tpdv), # 需要反归一化
            'actions': output_action.to(**self.tpdv),
            'mus' : mus.to(**self.tpdv),
            'sigmas' : sigmas.to(**self.tpdv)
        }
