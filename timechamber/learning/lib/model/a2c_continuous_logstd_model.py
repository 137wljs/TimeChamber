...
from timechamber.learning.lib.model.base_model import BaseModel
from timechamber.learning.lib.network.mlp import MLP
from timechamber.learning.lib.network.resnet import ResNetFeatureExtractor
from timechamber.learning.lib.network.cnn import CNNFeatureExtractor
...
class ModelA2CContinuousLogStd(BaseModel):
    ...
    def forward(self, input_dict):
        ...
        assert not torch.isnan(logstd).any(), "logstd contains NaN"
        assert not torch.isinf(logstd).any(), "logstd contains Inf"
        sigma = torch.exp(logstd)

        # if any element of std isn't positive, print it out
