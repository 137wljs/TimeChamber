...
import gym
import torch
from timechamber.learning.lib.core.segment_tree import SumSegmentTree, MinSegmentTree
from timechamber.learning.lib.core.torch_ext import numpy_to_torch_dtype_dict
...
class ExperienceBuffer:
    ...
    '''
    def __init__(self, env_info, algo_info, device, agent_idx, aux_tensor_dict=None):
        self.env_info = env_info
        self.algo_info = algo_info
        self.device = device

        self.agent_idx = agent_idx
        self.num_agents = env_info['num_agents'][agent_idx]
        self.action_space = env_info['action_space'][agent_idx]
        ...
        self.state_base_shape = (self.horizon_length, self.num_agents)
    ...
    def _init_from_env_info(self, env_info):
        ...
        obs_space = gym.spaces.Box(low=-np.Inf, high=np.Inf,shape=(env_info['observation_space'][self.agent_idx].shape[0],), dtype=np.float32)
        self.tensor_dict['obses'] = self._create_tensor_from_space(obs_space, obs_base_shape)
        
        val_space = gym.spaces.Box(low=0, high=1,shape=(env_info.get('value_size',1),))
        self.tensor_dict['outs'] = self._create_tensor_from_space(val_space, obs_base_shape)
